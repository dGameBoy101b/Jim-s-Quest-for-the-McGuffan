...
public class Hurt_Bolt_Controller : MonoBehaviour
{
    public float speed;
    public int damage;
    ...
    void FixedUpdate()
    {
        ...
        if (damage < 1)
        {
            Debug.Log("hurt bolt destroying self: "+id);
    ...
    {
        //reduce damage if collide with enemy heal bolt
        if (coll.gameObject.tag == enemy_heal_tag)
        {
            other_id = coll.gameObject.GetInstanceID();
            Debug.Log("collided with enemy heal bolt: "+other_id);
